...
from app.ai.scrapers.pharmacy import compare_prices, get_medicine_prices_tool
...
- Suggest consulting a doctor before switching brands"""
    ...
    async def process_async(self, query: str) -> str:
        """
        Process a user query about medicine prices (Async).
        """
...
Medicine name:"""
        ...
        medicine_name = (await self.ask_llm_async(prompt)).strip()
        
        if medicine_name == "NONE" or not medicine_name:
            # Try to help user identify medicines
            return await self._suggest_medicines_from_discharge_async(query)
        
        # Search for prices
        return await self._compare_prices_async(medicine_name, query)
    
    async def _suggest_medicines_from_discharge_async(self, query: str) -> str:
        """Suggest medicine names from discharge documents (Async)."""
        results = await self.search_discharge_async("medicine tablet capsule prescription", k=5)
        
        if not results:
            return """I couldn't find any medicines in this document to search for. Please either:

1. Specify the medicine name: "Compare prices for Paracetamol 500mg"
2. Ask me to find prices for a specific medication
...
## Response:"""
    ...
    async def _compare_prices_async(self, medicine_name: str, original_query: str) -> str:
        """Compare prices across platforms (Async)."""
        # Get price comparison (Run blocking scraper in threadpool)
        from starlette.concurrency import run_in_threadpool
        price_results = await run_in_threadpool(get_medicine_prices_tool, medicine_name)
...
## Response:"""
    ...
    def compare_all_prescribed(self) -> str:
        ...
        if not results:
            return "No medicines found in this document."
...
Medicine names (comma-separated):"""
        ...
        if not medicines:
            return "Could not extract medicine names from this document."
