...
Format responses clearly with tables and bullet points."""
    ...
    async def process_async(self, query: str) -> str:
        """
        Process a user query about their hospital bill (Async).
        """
        # Search user's bills
        bill_results = await self.search_bills_async(query, k=5)
        
        # Search regulations for context
        reg_results = await self.search_regulations_async(query, k=5)
        
        # Search insurance policies
        insurance_results = await self.search_insurance_async(query, k=5)
        
        if not bill_results:
            return """I couldn't find any relevant bill information in this document.
            
Please ensure this file is a hospital bill."""
...
## Response:"""
    ...
    def analyze_for_overcharging(self) -> str:
        ...
        if not unique_items:
            return "No bill items found for analysis in this document."
...
## Analysis Report:"""
    ...
    def get_bill_breakdown(self) -> str:
        ...
        if not results:
            return "No bill items found in this document."
